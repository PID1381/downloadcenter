...
def _show_cambio_url_menu() -> None:
    ...
    _CATS = [
        ("Anime (AW)",  "anime",    "animeworld"),
        ("Anime (AU)",  "anime",    "animeunity"),
    ...
    while True:
        ...
        for i, (label, cat, key) in enumerate(_CATS, 1):
            ...
            print(f"  {i}. {label:<12} {url}")
        print()
        scelta = ui.ask_choice(
            message="Scegli un'opzione",
            choices={
                "1": "🎌  Cambia URL Anime (AnimeWorld)",
                "2": "🎌  Cambia URL Anime (AnimeUnity)",
                "3": "📚  Cambia URL Manga",
                "4": "👗  Cambia URL Vinted",
                "5": "⬇️   Cambia URL Download",
                "6": "🔄  Reset URL di default",
                "0": "🔙  Torna",
            },
        )
...
def _submenu_anime_manga() -> None:
    while True:
        ui.show_header("🎌  ANIME E MANGA", "Menu principale > Anime e Manga")
        scelta = ui.ask_choice(
            message="Scegli un'opzione",
            choices={
                "1": "📺  Anime",
        ...
        if scelta == "0":
            ...
            try:
                ...
                _anime_mod.anime_handlers.show_menu()
...
def _submenu_impostazioni() -> None:
    while True:
        ui.show_header("⚙️   IMPOSTAZIONI", "Menu principale > Impostazioni")
        ...
        print(f"  ⏱️   Timeout (s):       {config.get_timeout()}")
        print()
        scelta = ui.ask_choice(
            message="Scegli un'opzione",
            choices={
                "1": "📂  Cambia download dir",
                "2": "🌐  Toggle headless browser",
                "3": "🐛  Toggle debug mode",
                "4": "🔗  Cambio URL moduli",
                "5": "🔄  Reset impostazioni default",
                "0": "🔙  Torna",
            },
        )
        if scelta == "0":
...
def _submenu_utilita() -> None:
    while True:
        ui.show_header("🛠️   UTILITÀ", "Menu principale > Utilità")
        scelta = ui.ask_choice(
            message="Scegli un'opzione",
            choices={
                "1": "🧹  Clear cache",
                "2": "💾  Backup file sensibili",
                "3": "♻️   Ripristina backup",
                "0": "🔙  Torna",
            },
        )
        if scelta == "0":
