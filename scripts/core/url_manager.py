...
Patch AnimeUnity: 28 Aprile 2026
  - Aggiunta voce "animeunity" in DEFAULT_URLS["anime"]
"""
import json
import re
...
class URLManager:
            ...
            try:
                ...
                if data and not any(isinstance(v, dict) for v in data.values()):
                    for key, val in data.items():
                        if any(k in key for k in ["animeworld", "animeclick", "animeunity"]):
                            base["anime"][key] = val
    ...
    def get(self, category: str, key: str = "") -> Optional[str]:
        """
        get("anime", "animeunity")  -> URL per categoria+chiave
        get("animeunity")           -> ricerca legacy in tutte le categorie
        """
        if key:
            return (
    ...
    def set(self, category: str, key: str, url: str = "") -> None:
        """
        set("anime", "animeunity", "https://...")   -> 3 argomenti
        set("animeunity", "https://...")             -> legacy 2 argomenti
        """
        if not url:
            url = key
