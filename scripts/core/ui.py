...
Changelog v2.4:
- visual_len(): larghezza visiva reale (emoji = 2 colonne, normali = 1)
- pad_to():     padding visivo corretto per emoji nei box
- ask_choice:   box dinamico con pieno supporto emoji nelle label
- show_header:  supporto emoji nel titolo
- Retrocompatibile con v2.3 / v2.2 / v2.1
"""
import os
import unicodedata
...
def visual_len(s: str) -> int:
    """Larghezza visiva di una stringa: emoji/wide = 2 colonne, altri = 1."""
    w = 0
    for ch in s:
...
def pad_to(s: str, width: int, fill: str = ' ') -> str:
    """Pad visivo: aggiunge 'fill' finché la larghezza visiva raggiunge width."""
