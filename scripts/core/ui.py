    ...
    def ask_choice(
        ...
        *,
        ...
        if choices is not None:
            ...
            # larghezza dinamica: adatta il box alla label più lunga
            _kw = max(len(str(k)) for k in choices.keys())
            _lw = max(len(str(v)) for v in choices.values())
            # riga: "  |  " + key(_kw) + ".  " + label(_lw) + "  |"
            # inner (tra i due +) = 2 + _kw + 2 + 2 + _lw + 2 = _kw + _lw + 8
            _inner  = _kw + _lw + 8
            _border = "  +" + "-" * _inner + "+"
            def _row(k, v):
                return f"  |  {str(k):<{_kw}}.  {str(v):<{_lw}}  |"
            print(_border)
