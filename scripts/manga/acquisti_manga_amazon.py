...
def _fetch_amazon_search(query: str) -> list[dict]:
    ...
    FIX v2.5: recovery ultra-rapido pagina 'Ci dispiace':
      1. Tenta URL diretto di ricerca
      2. Se rileva pagina errore → goto immediato a BASE_URL (no click)
      3. Attende 300 ms + dismiss cookie → ripete ricerca
    """
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                # ── Recovery ultra-rapido: pagina "Ci dispiace" ────────────
                if _is_error_page(html):
                    print("  [!] Pagina errore rilevata. Recovery rapido...")

                    # Goto diretto alla home (no attesa click)
                    ...
                    # Ripeti ricerca immediatamente
                    print(f"  [->] Ripeto ricerca '{query}'...")
                    page.goto(url, wait_until="domcontentloaded")
                    _dismiss_cookie(page)
                    _wait_for_results(page)
                    _dismiss_cookie(page)
