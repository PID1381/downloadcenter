...
def _print_results(items: list[dict]) -> None:
    """Tabella risultati Amazon con box-drawing standard ┌─┬┐."""
    headers = ["N", "Titolo", "Prezzo"]
    col_w   = [3, 44, 10]
    ...
    for i, it in enumerate(items, 1):
        p  = it.get("prezzo", "—") or "—"
        nd = " *" if it.get("disponibile") == "Non disponibile" else ""
        print(_row([str(i), it["titolo"] + nd, p]))
    print(_border("└","┴","┘","─"))
