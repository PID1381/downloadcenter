...
# -*- coding: utf-8 -*-
"""
acquisti_manga_amazon.py v2.4
==============================
Modulo per la ricerca manga su Amazon.it.
Percorso: scripts/manga/acquisti_manga_amazon.py

NOVITA v2.3:
  - _extract_results ora estrae il campo "asin" dall'URL del prodotto
    (es. /dp/8822617899/ → asin = "8822617899")
  - Usato da ricerca_automatica_acquisti per il controllo ISBN italiano
"""
...
def _extract_asin(url: str) -> str:
    """
    Estrae l'ASIN dal percorso /dp/XXXXXXXXXX/ dell'URL Amazon.
...
def _get_folder() -> str | None:
    try:
        prefs = load_prefs()
        base = prefs.get("default_export_dir", "").strip()
    except Exception:
        base = ""
...
def _wait_for_results(page) -> bool:
    for sel in [
        ...
        try:
            ...
            continue
    try:
        page.wait_for_load_state("networkidle", timeout=4_000)  # FIX v5.5: ridotto da 8 s a 4 s
    except Exception:
        pass
...
def _is_error_page(html: str) -> bool:
    """Rileva la pagina 'Ci dispiace' / errore generico di Amazon."""
    lower = html.lower()
    return any(t in lower for t in ["ci dispiace", "si è verificato un errore", "tornare alla home"])


def _fetch_amazon_search(query: str) -> list[dict]:
    """
    Apre Amazon.it, esegue la ricerca e restituisce i risultati.
    Ogni risultato include il campo "asin" estratto dall'URL.
    Usata anche da ricerca_automatica_acquisti.py.

    FIX v2.4: recovery automatico pagina 'Ci dispiace':
      1. Tenta URL diretto di ricerca
      2. Se rileva pagina errore → clicca home o naviga a BASE_URL
      3. Attende (800 ms) + dismiss cookie → ripete ricerca
    """
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                # ── Recovery: pagina "Ci dispiace" ─────────────────────────
                if _is_error_page(html):
                    print("  [!] Amazon ha restituito pagina di errore. Recovery in corso...")

                    # Step 1: clicca link home se presente, altrimenti goto diretto
                    try:
                        page.click("a[href*='amazon.it']", timeout=1_500)
                    except Exception:
                        page.goto(BASE_URL, wait_until="domcontentloaded")

                    # Step 2: attesa minima + dismiss cookie
                    page.wait_for_timeout(800)
                    _dismiss_cookie(page)
                    page.wait_for_timeout(200)

                    # Step 3: ripeti ricerca
                    print(f"  [->] Ripeto ricerca '{query}' dalla home...")
                    try:
                        page.goto(url, wait_until="domcontentloaded")
                    except Exception:
    ...
    if _is_error_page(html):
        print("  Amazon continua a mostrare la pagina di errore dopo il recovery.")
        print("  Riprova tra qualche secondo.")
        return []
...
def _extract_results(html: str) -> list[dict]:
    ...
    for item in items:
        ...
        results.append({
            ...
            "disponibile": disponibile,
            "url":         url,
            "asin":        asin,   # ← NUOVO: ISBN-10 del prodotto
        })
