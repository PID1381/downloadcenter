...
# -*- coding: utf-8 -*-
"""
MANGA ENGINE v1.5 - CORE ENGINE CENTRALIZZATO + URL MANAGER PATCH
Download Center - scripts/manga/manga_engine.py

PATCH v1.5 (APPLICATA):
  § URL Manager Integration
    - Import url_mgr con fallback
    - BASE_URL_MCM, BASE_URL_ACK, BASE_URL_AMZ da url_mgr
    - Fallback a hardcoded se url_mgr non disponibile
    - 100% backward-compatible

NOVITA v1.4:
  §9.5 Metodo MangaPageSession.fetch_animeclick_manga_search() aggiornato:
        - Parametro author: str = "" aggiunto
        - Supporto campo #search_manga_staff (autore)
        - Logica: compila titolo e/o autore in base ai parametri
...
# ── URL base siti manga (con PATCH v1.5: URL Manager) ──────────────────────

if HAS_URL_MGR and url_mgr:
    BASE_URL_MCM = url_mgr.get("mangacomicsmarket") or "https://www.mangacomicsmarket.it"
    BASE_URL_ACK = url_mgr.get("animeclick") or "https://www.animeclick.it"
    BASE_URL_AMZ = url_mgr.get("amazon") or "https://www.amazon.it"
else:
    BASE_URL_MCM = "https://www.mangacomicsmarket.it"
    BASE_URL_ACK = "https://www.animeclick.it"
    BASE_URL_AMZ = "https://www.amazon.it"
...
MANGA_AMZ_QUERY = "https://www.amazon.it/s?k={query}&i=" + MANGA_AMZ_DEPT
...
class MangaPageSession:
    ...
    def fetch_animeclick_manga_search(self, query: str, author: str = "") -> List[Dict]:
        """
        Ricerca manga su AnimeClick usando Playwright.
        URL: https://www.animeclick.it/ricerca/manga
        Campi ricerca: #search_manga_title (titolo), #search_manga_staff (autore)
        Risultati: div#row-elenco-opere > div.thumbnail-opera-info-extra

        v1.4: Aggiunto supporto ricerca per autore (opzionale)
        - query: titolo manga (opzionale se author presente)
        - author: autore manga (opzionale se query presente)
        """
        if not self._page:
        ...
        try:
            ...
            search_field_title = "#search_manga_title"
            search_field_author = "#search_manga_staff"
            
            # Attendi il campo titolo
            self._page.wait_for_selector(search_field_title, timeout=10_000)
            
            # Compila titolo se presente
            if query:
                self._page.click(search_field_title)
                self._page.wait_for_timeout(200)
                self._page.fill(search_field_title, "")
                self._page.wait_for_timeout(150)
                self._page.fill(search_field_title, query)
                self._page.evaluate(
                    """(sel) => {
            ...
            # Compila autore se presente
            if author:
                try:
                    self._page.wait_for_selector(search_field_author, timeout=5_000)
                    self._page.click(search_field_author)
                    self._page.wait_for_timeout(150)
                    self._page.fill(search_field_author, "")
                    self._page.wait_for_timeout(100)
                    self._page.fill(search_field_author, author)
                    self._page.evaluate(
                        """(sel) => {
                            const el = document.querySelector(sel);
                            if (el) {
                    ...
                    pass  # Campo autore potrebbe non essere sempre disponibile
            
            # Esegui ricerca
            self._page.wait_for_timeout(300)
            self._page.press(search_field_title, "Enter")
...
__all__ = [
    ...
    "extract_animeclick_manga_results",
    # §10 — Collection (4)
...
if __name__ == "__main__":
    print("=" * 56)
    print("  MANGA ENGINE v1.5 — URL Manager Patch Applied")
    print("=" * 56)
    print()
    print(f"  Esportazioni __all__  : {len(__all__)}")
    print(f"  HAS_REQUESTS          : {HAS_REQUESTS}")
    print(f"  HAS_PLAYWRIGHT        : {HAS_PLAYWRIGHT}")
    print(f"  HAS_URL_MGR           : {HAS_URL_MGR}")
    print()
    print("  URL Manager Integration:")
    print(f"    BASE_URL_MCM  : {BASE_URL_MCM}")
    print(f"    BASE_URL_ACK  : {BASE_URL_ACK}")
    BASE_URL_AMZ  : {BASE_URL_AMZ}")
    print()
    print("  Fallback: automatico a hardcoded se url_mgr non disponibile")
