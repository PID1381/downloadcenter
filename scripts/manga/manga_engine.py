...
VINTED_EXPORT_FOLDER = "Vinted"   # sottocartella export inserzioni Vinted
_engine_get_export_dir = None          # inizializzato; sovrascritto da ricerca_vinted.py
def _read_prefs_export_dir() -> str:
    """Legge export_dir direttamente da prefs.json (fallback robusto)."""
    try:
        with open(_PREFS_FILE, "r", encoding="utf-8") as fh:
            data = json.load(fh)
        return data.get("default_export_dir", "") or ""
    except Exception:
        return ""
...
def _print_catalog_list(items: list) -> None:
    """Tabella risultati catalogo MCM con box-drawing standard ┌─┬┐."""
    COL_N  = 4
    COL_T  = 50
    COL_P  = 10
    col_w  = [COL_N, COL_T, COL_P]
    indent = "  "
    ...
    def _border(l, m, r, f):
        return indent + l + m.join(f*(w+2) for w in col_w) + r
    ...
    print()
    print(_border("┌","┬","┐","─"))
    print(_row(["N", "Titolo", "Prezzo"]))
    print(_border("├","┼","┤","─"))
    for i, item in enumerate(items, 1):
        flag  = " ▶ " if item.get("preordine") else "   "
        full  = _format_title_with_preorder(item)
        title = flag.strip() + " " + _truncate_title_smart(full, max_title_len=47) if item.get("preordine") else _truncate_title_smart(full, max_title_len=50)
        print(_row([str(i), title, item.get("prezzo", "")]))
    print(_border("└","┴","┘","─"))
    print()
    total = len(items); pre_n = sum(1 for it in items if it.get("preordine"))
    print(f"  Totale: {total}  (disponibili: {total - pre_n}  |  preordini: {pre_n})")
