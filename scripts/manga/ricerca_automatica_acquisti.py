...
# -*- coding: utf-8 -*-
"""
ricerca_automatica_acquisti.py v5.9 — fix MCM ordine + filtro Amazon ISBN
# FIX v5.9:
#   F1 - ricerca_mcm.py: _wait_for_catalog_items usa auto_timeout_ms=4s in AUTO
#        Evita blocco su titoli con risultati generici MCM
#   F2 - acquisti_manga_amazon.py: networkidle timeout 8s -> 4s
=================================================================
Novita v5.2 (fix):
...
def handle_ricerca_automatica() -> None:
    ...
    try:
        for i, manga in enumerate(selected_manga, 1):
            ...
            _filled = int((i - 1) / total * 40)
            _bar    = "█" * _filled + "░" * (40 - _filled)
            _chars  = ["◐","◓","◑","◒"]
            _anim   = _chars[int(_t.time() * 4) % 4]
            print(f"\r      Analisi: |{_bar}| {i}/{total}  {label:<34} {_anim}", end="", flush=True)
