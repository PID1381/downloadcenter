...
# -*- coding: utf-8 -*-
"""
ricerca_mcm.py v5.6 — 1 pagina x 96 risultati (AUTO e MANUAL)
# FIX v5.6: early-exit via div 'Nessun prodotto trovato' (race condition)
...
def _wait_for_catalog_items(page, auto_timeout_ms: int | None = None) -> None:
    # FIX v5.5: timeout ridotto in AUTO (4 s invece di 12 s)
...
def _fetch_search_pages(
    ...
    mode = "auto"   -> MAX_PAGES_AUTO=1, TARGET_ITEMS_AUTO=96,
                       URL diretta &term=, no prompt interattivi
    mode = "manual" -> MAX_PAGES_MANUAL=1, TARGET_ITEMS_MANUAL=96,
                       URL diretta &term= con fallback _perform_search,
                       prompt interattivi attivi

    P1: Tenta sempre prima l'URL diretta con &term=.
    ...
    while True:
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    # P1: goto con &term= gia nell'URL
                    pg.goto(search_url, wait_until="domcontentloaded")
                    _dismiss_cookie_mcm(pg)

                    # ── FIX v5.6: race condition ─────────────────────────────────────────
                    # Aspetta il primo tra: card__name presenti  OPPURE  div "Nessun prodotto trovato"
                    # Evita i doppi wait_for_timeout fissi che causavano lentezza su titoli assenti.
                    _wait_ms = 4_000 if is_auto else 12_000
                    try:
                        ...
                        pass  # timeout → lascia decidere al controllo successivo

                    # Verifica cosa ha vinto la race
                    _found_empty = pg.evaluate(
                        """() => Array.from(document.querySelectorAll('.text-grey-strong'))
                            .some(el => el.textContent.includes('Nessun prodotto trovato'))"""
                    )
                    _found_cards = pg.evaluate("""() =>
                        Array.from(document.querySelectorAll('p.card__name'))
                            .filter(p => {
                                const cls = Array.from(p.classList);
                                return cls.includes('card__name') && !cls.includes('card__name--big');
                            }).length > 0
                    ...
                    if _found_empty and not _found_cards:
                        # Nessun risultato confermato dal div MCM
                        if not is_auto:
                            # MANUAL: prova fallback _perform_search
                            ...
                            # AUTO: esci subito, zero attese extra
                            return []
                    ...
                    for pg_num in range(1, max_pages + 1):
                        if pg_num > 1:
                            if not is_auto:
                                _select_96_per_page(pg)
                            _wait_for_catalog_items(pg, auto_timeout_ms=4_000 if is_auto else None)  # FIX v5.5
