...
# -*- coding: utf-8 -*-
"""
ricerca_mcm.py v5.5 — 1 pagina x 96 risultati (AUTO e MANUAL)
# FIX v5.5: _wait_for_catalog_items usa auto_timeout_ms=4000 in modalità AUTO
...
def _wait_for_catalog_items(page, auto_timeout_ms: int | None = None) -> None:
    ...
    for sel in ["p.card__name", ".card__name", "[class*='card__name']"]:
        try: page.wait_for_selector(sel, timeout=12_000); page.wait_for_timeout(wait_after); return
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    # FIX v5.4: in AUTO, 0 risultati = titolo assente su MCM → esci subito
                    if quick_count == 0 and is_auto:
                        return []
