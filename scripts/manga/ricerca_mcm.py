...
def _wait_for_catalog_items(page, auto_timeout_ms: int | None = None) -> None:
    # FIX v5.6: timeout ridotto in AUTO (4 s invece di 12 s)
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    # Aspetta cards OPPURE div "Nessun prodotto trovato" (whichever comes first)
                    ...
                    if _found_empty and not _found_cards:
                        if not is_auto:
                            ...
                            return []  # FIX v5.6: esci subito, zero attese extra
                    if _found_empty and not _found_cards:
                    ...
                    for pg_num in range(1, max_pages + 1):
                        if pg_num > 1:
                            if not is_auto:
                                _select_96_per_page(pg)
                            _wait_for_catalog_items(pg, auto_timeout_ms=4_000 if is_auto else None)  # FIX v5.6
