...
# -*- coding: utf-8 -*-
"""
la_mia_collezione.py v2.5 — RICERCA PER TITOLO + AUTORE
=========================================================
Modulo per La mia collezione manga.
Percorso: scripts/manga/la_mia_collezione.py

PATCH v2.5 (rispetto a v2.4):
  NUOVA FUNZIONALITA':
    - Ricerca singola: menu di scelta ricerca
      T  Solo titolo
      A  Solo autore
      TA Titolo + Autore
      0  Torna
    
    - Campi di input dinamici in base alla scelta
    - Passa entrambi i parametri a tracker.search_manga(query, author)
    - Monkey-patch aggiornato per supportare author
    - Ricerca multipla: mantiene ricerca per titolo (non estesa)

PATCH v2.4:
  - Aggiunta _print_manga_detail(): visualizza dettagli completi
  - Menu dopo dettagli: "S Salva | 0 Torna"

PATCH v2.3:
  - search_manga() usa MangaPageSession + Playwright
  - Fix ricerca robusta per titoli come "Maison Ikkoku"
"""
...
# ── Monkey-patch v2.5: search_manga() con supporto autore ───────────────────
if _IMPORT_OK and _AnimeTracker is not None:
    if not hasattr(_AnimeTracker, "search_manga"):
        def _search_manga(self, query: str, author: str = "", silent: bool = False) -> list:
            """
            Cerca manga su AnimeClick usando Playwright.
            URL: https://www.animeclick.it/ricerca/manga
            Campi: #search_manga_title (titolo), #search_manga_staff (autore)
            Risultati: #row-elenco-opere > div.thumbnail-opera-info-extra
            
            v2.5: Aggiunto supporto ricerca per autore (opzionale)
            Aggiunto via monkey-patch da la_mia_collezione.py v2.5.
            """
            ...
            try:
                with MangaPageSession() as sess:
                    return sess.fetch_animeclick_manga_search(query, author)
...
def _print_manga_detail(details: dict) -> None:
    """Visualizza TUTTI i dettagli disponibili del manga in formato leggibile."""
    print()
    print(_EQ)
    
    titolo = (details.get("titolo") or details.get("title") or details.get("titolo_originale", "")).strip()
    if len(titolo) > WIDTH - 4:
        titolo = titolo[:WIDTH - 7] + "..."
    print(f"  {titolo}")
    print(_EQ)
    
    titolo_orig = details.get("titolo_originale", "").strip()
    if titolo_orig and titolo_orig.lower() != titolo.lower():
        print(f"  {'Titolo originale':<26} {titolo_orig}")
    
    anno = details.get("anno", "").strip()
    if anno:
        print(f"  {'Anno':<26} {anno}")
    
    voto = details.get("voto", "").strip()
    if voto:
        print(f"  {'Voto':<26} {voto}")
    
    generi = details.get("generi", [])
    if generi:
        if isinstance(generi, list):
            generi_str = ", ".join(str(g) for g in generi if g)
        else:
            generi_str = str(generi)
        if generi_str:
            print(f"  {'Generi':<26} {generi_str}")
    
    stato = details.get("stato_italia", "").strip() or details.get("stato", "").strip()
    if stato:
        print(f"  {'Stato in Italia':<26} {stato}")
    
    volumi = details.get("volumi", "").strip()
    if volumi:
        print(f"  {'Volumi':<26} {volumi}")
    
    autore = details.get("autore", "").strip()
    if autore:
        print(f"  {'Autore':<26} {autore}")
    
    casa_editrice = details.get("casa_editrice", "").strip()
    if casa_editrice:
        print(f"  {'Casa editrice':<26} {casa_editrice}")
    
    descrizione = details.get("descrizione", "").strip()
    if descrizione:
        print()
        print("  Descrizione:")
        max_w = WIDTH - 6
    ...
    link = details.get("link", "").strip() or details.get("url", "").strip()
    if link:
        print()
        print("  Link:")
        print(f"  {link}")
...
# ── Ricerca per titolo (MODIFICATO v2.5) ──────────────────────────────────────
...
def _handle_singola(tracker) -> None:
    """v2.5: Aggiunto menu di scelta ricerca (titolo / autore / titolo+autore)."""
    while True:
        clear_screen(); print("  " + _EQ)
        ...
        print("    T   Solo titolo")
        print("    A   Solo autore")
        print("    TA  Titolo + Autore")
        print("    0   Torna")
        print()
        tipo = input("  Scelta (T/A/TA/0): ").strip().upper()
        
        if tipo == "0":
            return
        elif tipo not in ("T", "A", "TA"):
            show_error("Opzione non valida.")
            input("  Premi INVIO...")
            continue
        
        # Raccogli input in base al tipo di ricerca
        query = ""
        author = ""
        ...
        if tipo in ("A", "TA"):
            author = input("  Autore da cercare (0 = torna): ").strip()
            if not author or author == "0":
                continue
        
        if not query and not author:
            show_error("Almeno uno tra titolo e autore è obbligatorio.")
            input("  Premi INVIO...")
            continue
        ...
        if tipo == "T":
            ...
            search_desc = f"autore '{author}'"
        else:
            search_desc = f"'{query}' - autore '{author}'"
        
        print(f"\n  Ricerca {search_desc} in corso...")
        results = tracker.search_manga(query, author, silent=True)
        ...
        while True:
            clear_screen(); print("  " + _EQ)
            print(f"  RISULTATI [Manga]: {search_desc}  ({len(results)} trovati)")
            print("  " + _EQ); print()
            ...
            _print_manga_detail(details)
            
            print()
            print("  " + _SEP)
            print("  S  Salva in collezione  |  0  Torna ai risultati")
            print("  " + _SEP)
...
def _handle_multipla(tracker) -> None:
    """Ricerca multipla: mantiene ricerca per titolo (non estesa a autore)."""
    while True:
        clear_screen(); print("  " + _EQ)
            ...
            if _build_groups_fn and _print_groups_fn and _parse_multi_sel_fn:
                ...
                if sc.isdigit() and 1 <= int(sc) <= len(flat):
                    ...
                    if details:
                        ...
                        print()
                        print("  " + _SEP)
                        print("  S  Salva in collezione  |  0  Torna")
                        print("  " + _SEP)
                        sc2 = input("  Scelta: ").strip().upper()
                        if sc2 == "S":
                            _prompt_save(details)
...
def _handle_url_diretto(tracker) -> None:
    while True:
        ...
        _print_manga_detail(details)
        print(); print("  " + _SEP)
        print("  S  Salva in collezione  |  1  Inserisci un altro URL  |  0  Torna"); print("  " + _SEP)
        sc = input("  Scelta: ").strip().upper()
        if sc == "S":
            _prompt_save(details)
            input("\n  Premi INVIO...")
