...
try:
    from manga_engine import (
        ...
        get_collection_path, get_link_dir, get_export_dir,
    ...
    def get_export_dir():
        ...
        try:
            with open(_TEMP_DIR / 'prefs.json', encoding='utf-8') as _f:
                return _json.load(_f).get('default_export_dir', '')
        except Exception: return ''
...
try:
    from scripts.manga import manga_engine as ricerca_scheda_anime  # FIX opzB as _rsa
...
def _get_export_folder() -> str | None:
    base = get_export_dir()
    if not base:
        print("\n  Percorso 'default_export_dir' non configurato.")
