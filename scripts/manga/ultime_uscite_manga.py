...
# -*- coding: utf-8 -*-
"""
ultime_uscite_manga.py v2.0 — REFACTORED
=========================================
Modulo per le Ultime uscite manga di AnimeClick.it.
Percorso: scripts/manga/ultime_uscite_manga.py

REFACTORING v2.0 (rispetto a v2.1):
  - Import da manga_engine: clear_screen, sanitize_filename, get_link_dir,
    get_headless_mode, get_page_timeout, load_prefs, save_prefs,
    WIDTH, COOKIE_SEL, COOKIE_TEXTS
  - Rimossi: _load_prefs, _save_prefs, _get_headless, _get_page_timeout,
             _get_link_dir, _clear, _safe_filename
  - _new_page() / _handle_intro_redirect() usano funzioni engine
  - Path assoluta via Path(__file__)
"""
...
def _handle_intro_redirect(page, original_url: str) -> None:
    """Gestisce redirect AnimeClick -> /video-intro."""
...
def _extract_news_list(html: str) -> list:
    ...
    for col in news_div.find_all("div", class_="col-news")[:5]:
        thumb = (col.find("div", class_=lambda c: c and "thumbnail-news" in c)
                 or col.find("div", class_=lambda c: c and "thumbnail" in c) or col)
        titolo = ""; link_href = ""
        ...
        if caption:
            for tag in ["h4","h3","h2","h5","p"]:
                ...
                if h:
                    ...
                    if a: titolo = a.get_text(strip=True); link_href = a.get("href","")
                    else: titolo = h.get_text(strip=True)
                    break
            if not titolo:
                for a in caption.find_all("a", href=True):
                    t = a.get_text(strip=True)
                    if t and len(t) > 2: titolo = t; link_href = a.get("href",""); break
        if not titolo:
            for tag in ["h4","h3","h2","h5"]:
                ...
                if h:
                    ...
                    break
        if not titolo:
            il = thumb.find("a", href=True)
            if il:
                t = il.get("title","").strip()
                if t and len(t) > 2: titolo = t; link_href = il.get("href","")
        if not titolo:
            img = thumb.find("img")
            if img:
                t = img.get("alt","").strip()
                if t and len(t) > 2: titolo = t
                if not link_href:
                    pa = img.find_parent("a", href=True)
                    if pa: link_href = pa.get("href","")
        if not titolo:
            for a in col.find_all("a", href=True):
                t = a.get_text(strip=True)
                if t and len(t) > 3: titolo = t; link_href = a.get("href",""); break
        if not titolo: continue
        data = ""
        tt = thumb.find("time")
        if tt: data = tt.get("datetime","") or tt.get_text(strip=True)
        if not data:
            for tag in (caption or thumb).find_all(["small","span","em","abbr","p"]):
                txt = tag.get_text(strip=True)
                if re.search(r"\d{1,2}[/\-\.]\d{1,2}", txt): data = txt; break
        results.append({"titolo": titolo, "data": data, "link": _normalise(link_href)})
    return results
...
def _parse_tab_edizioni(table) -> list:
    ...
    for row in rows[1:]:
        ...
        entries.append({"titolo": t, "data": _gc(cells,"data"), "prezzo": _gc(cells,"prezzo"),
                        "casa_editrice": _gc(cells,"editore"), "link": _gl(cells)})
    return entries

def _extract_article_groups(html: str) -> dict:
    groups = {g: [] for g in _MANGA_GROUPS_ORDER}
    soup = BeautifulSoup(html, "html.parser")
    body = (soup.find("div", class_=re.compile(r"news.?body|corpo.?news|news.?content|contenuto", re.I))
            or soup.find("article") or soup.find("div", class_="corpo")
            or soup.find("div", class_=re.compile(r"^content$|^corpo$", re.I)) or soup)
    current_group = "Uscite ufficiali"; seen: set = set()
    for el in body.descendants:
        if not hasattr(el, "name") or not el.name: continue
        if el.name in ("strong","b","h2","h3","h4"):
            txt = el.get_text(strip=True).lower()
            if "lost in action" in txt: current_group = "Lost in action"
            elif re.search(r"data non pervenuta|data n\.p\.|senza data", txt): current_group = "Data non pervenuta"
            elif re.search(r"uscite ufficiali|uscite del mese|uscite confermate", txt): current_group = "Uscite ufficiali"
        if el.name == "div" and "tab-edizioni-wrap" in " ".join(el.get("class") or []):
            ...
            if table and id(table) not in seen:
                seen.add(id(table))
                for e in _parse_tab_edizioni(table): e["gruppo"] = current_group; groups[current_group].append(e)
        if el.name == "table":
            cls = " ".join(el.get("class") or [])
            if ("tab-edizioni" in cls or el.find("th") or el.find("td")) and id(el) not in seen:
                seen.add(id(el))
                for e in _parse_tab_edizioni(el): e["gruppo"] = current_group; groups[current_group].append(e)
    return groups
...
def _print_news_list(news: list) -> None:
    ...
    for i, art in enumerate(news, 1):
        print("  {:<4} {}{}".format(i, art["titolo"], "  ["+art["data"]+"]" if art.get("data") else ""))
    print(_SEP)
...
def _print_groups_summary(groups: dict) -> dict:
    ...
    for gn in _MANGA_GROUPS_ORDER:
        ...
        for entry in entries:
            ...
            lnk = entry.get("link",""); title_str = _hyperlink(entry["titolo"], lnk) if lnk else entry["titolo"]
    ...
    while True:
        ...
        for part in [p.strip() for p in raw.split(",") if p.strip()]:
            if "-" in part:
                ...
                except ValueError: print("  Range non valido: '{}' \u2014 ignorato.".format(part)); has_error = True
            else:
                try:
                    n = int(part)
                    if n in item_map:
                ...
                except ValueError: print("  Valore non valido: '{}' \u2014 ignorato.".format(part)); has_error = True
    ...
    while True:
        ...
        if n_sel == 1:
            ...
            if azione == "1":
                ...
                folder = _ensure_uscite_manga_folder()
                if folder: _write_article_file(folder, selected, article["titolo"]); print("\n  {} titoli in: {}".format(n_sel, folder))
                input("\n  Premi invio..."); item_map = _reprint()
            elif azione == "3": item_map = _reprint()
            else: print("  Opzione non valida.")
...
def handle_manga_uscite() -> None:
    ...
    clear_screen(); print(_EQ); print("  ULTIME USCITE MANGA  AnimeClick.it"); print(_EQ)
    print("\n  URL di default:\n  " + MANGA_USCITE_URL)
    ...
    if not html: print("\n  Errore: impossibile caricare la pagina."); input("  Premi invio per tornare..."); return
    news = _extract_news_list(html)
    if not news: print("\n  Nessun articolo trovato."); input("  Premi invio per tornare..."); return

    def _show_list():
        clear_screen(); print(_EQ); print("  ULTIME USCITE MANGA  AnimeClick.it"); print(_EQ)
        print("  URL: " + url); _print_news_list(news)
