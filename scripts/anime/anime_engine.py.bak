...
def _au_open_searchbar(page, ctx=None) -> bool:
    ...
    PROBLEMA: AnimeUnity apre popup/pubblicita al click sulla lente.
    Il popup apre una nuova scheda (YouTube ecc.) e Playwright
    perde il riferimento alla pagina principale -> crash.
    
    SOLUZIONE:
      1. Blocca route pubblicitarie via page.route() prima del click
      2. Registra ctx.on("page") che chiude ogni nuova scheda
      3. Dopo il click chiama _au_close_popups(ctx)
      4. Chiama page.bring_to_front() per riportare il focus
    
    Returns: True se input.search-bar e visibile, False altrimenti.
    """
    INPUT_SELS = [
        "input.search-bar",
    ...
    # Controlla se il campo e gia visibile (caso raro)
    ...
    # Blocca richieste pubblicitarie che aprono nuove schede
    try:
    ...
    # Listener automatico: chiude ogni nuova scheda appena aperta
    if ctx:
        try:
    ...
    # Clicca il pulsante lente con i selettori CSS
    clicked = False
    for sel in BTN_SELS:
        try:
            el = page.query_selector(sel)
    ...
    # Fallback: click via JavaScript (piu sicuro contro popup)
    if not clicked:
        if _AU_DEBUG:
        ...
        try:
            ...
            if _AU_DEBUG:
                print(f"  [AU_DEBUG] errore fallback JS: {_ex}")
    ...
    # Debug: screenshot + dump DOM + lista button/input
    if _AU_DEBUG:
        try:
            html = page.content()
