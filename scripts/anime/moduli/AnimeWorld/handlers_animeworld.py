...
def _build_dub_param(core) -> str:
    ...
    Legge da config.get('anime.lang', 'all').
    """
    lang = core.config.get('anime.lang', 'all')
    if lang == 'ita':
        return '&dub=1'
    if lang == 'sub':
        return '&dub=0'
...
def _ask_index(core, items: list, prompt: str = 'Seleziona: ') -> int | None:
    ...
    """
    scelta = core.ui.ask(prompt).strip()
    if scelta == '0':
        return None
...
def _dettaglio(core, item: dict) -> None:
    ...
    from scripts.core import handlers_watchlist, link_extractor   # TODO: adatta import al tuo percorso
...
def _ricerca(core) -> None:
    ...
    titolo = core.ui.ask('Titolo da cercare: ').strip()
    if not titolo:
        return
    ...
    core.ui.spinner_start('Ricerca in corso...')
    try:
...
def run(core) -> None:
    ...
    Chiamato dal dispatcher principale quando l'utente seleziona AnimeWorld.
    """
    base_url = core.url_manager.get_url(MODULE_KEY, 'base_url')
    if not base_url:
        core.ui.stampa(
    ...
    if not _ensure_browser(core):
        core.ui.stampa('⚠  Impossibile avviare il browser (Playwright).', 'warn')
        return

    while True:
        core.ui.titolo(MODULE_NAME)
        scelta = core.ui.menu([
            ('1', 'Ultime uscite'),
            ('2', 'Ricerca'),
            ('0', 'Indietro'),
...
# ═════════════════════════════════════════════════════════════
# CONTRATTI PUBBLICI — SILENT
# Nessun output, nessuna eccezione non gestita.
# ═════════════════════════════════════════════════════════════
