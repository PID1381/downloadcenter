...
def _au_livesearch_js() -> str:
    ...
    AnimeUnity usa Vue.js: i risultati appaiono in:
      <div class="results">
        <div class="result">
          <a class="livesearch-item" href="/anime/{id}-{slug}">
            <div class="livesearch-name">
              <div class="livesearch-title">Titolo</div>
              <span class="livesearch-info">Anno</span>
              <span class="livesearch-info info-2">Tipo</span>
              <span class="livesearch-info info-2"> • N/TOT episodi</span>

    Ritorna array JSON di oggetti {title, link, year, type, episodes}.
    """
    return """() => {
        const results = [];
        document.querySelectorAll('.results .result').forEach(r => {
            ...
            if (!a) return;
            const href  = a.href || '';
            const title = (r.querySelector('.livesearch-title') || {}).innerText || '';
            ...
            const eps   = infos[2] ? infos[2].innerText.replace('\u2022','').trim() : '';
            if (title && href) {
...
def _au_open_searchbar(page) -> bool:
    ...
    Flusso UI reale AnimeUnity:
      1. Clicca <button class="input-group-text btn-dark-gray2">
         (contiene <i class="fas fa-search">)
      2. Il campo <input class="search-bar"> diventa visibile/attivo
      3. Digita il titolo → risultati in .results .result (live, senza submit)

    Returns:
        True se il campo e stato aperto con successo, False altrimenti.
    """
    # Selettori per il pulsante lente (in ordine di priorita)
    SEARCH_BTN_SELS = [
    ...
    # Selettori per il campo input (per verificare che sia visibile)
    SEARCH_INPUT_SELS = [
        "input.search-bar",
    ...
    # Prima prova: il campo potrebbe essere gia visibile (senza cliccare)
    ...
    # Clicca il pulsante lente per aprire il campo
    clicked = False
    ...
    if not clicked:
        # Ultimo tentativo: click via JS sull'icona fa-search
        try:
            page.evaluate("""() => {
                const btn = document.querySelector(
                    'button.btn-dark-gray2, button[type=\"submit\"].input-group-text'
                );
                if (btn) btn.click();
                else {
                    const ico = document.querySelector('i.fa-search, .fas.fa-search');
                    if (ico) ico.closest('button') && ico.closest('button').click();
                }
            }""")
            page.wait_for_timeout(600)
        except Exception:
    ...
    # Verifica che il campo sia ora visibile
    for sel in SEARCH_INPUT_SELS:
        try:
...
def search_animeunity(query: str, silent: bool = False) -> List[Dict]:
    ...
    Flusso UI reale AnimeUnity:
      1. Clicca il pulsante lente  <button class="input-group-text btn-dark-gray2">
      2. Il campo <input class="search-bar"> diventa attivo
      3. Digita il titolo (delay 80ms per sembrare umano)
      4. I risultati appaiono in <div class="results"> senza premere invio
      5. Legge .results .result > a.livesearch-item
        ...
        try:
            ...
            if app_div:
                ...
                if data_page:
                    ...
                    fast_results = []
                    for a in animes:
                        title    = a.get("title_it") or a.get("title") or ""
                        slug     = a.get("slug", "")
                        anime_id = a.get("id", "")
                        if title and slug and anime_id:
                            ...
                            fast_results.append({
                                ...
                                "year":      str(a.get("date", "")),
                                "type":      a.get("type", ""),
                                "episodes":  str(a.get("episodes_count", "")),
                            })
                    if fast_results:
                        return fast_results
        except Exception:
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                # 2. Clicca il pulsante lente per aprire il campo di ricerca
                bar_open = _au_open_searchbar(page)
                if not bar_open:
                    if not silent:
                        show_warning("Pulsante ricerca AnimeUnity non trovato.")
                    browser.close()
                    return []

                # 3. Trova il campo input e digita il titolo
                input_sel = None
                for sel in [
                    "input.search-bar",
                    "input[placeholder*='Cerca']",
                ...
                if not input_sel:
                    if not silent:
                        show_warning("Campo input.search-bar non trovato dopo click.")
                    browser.close()
                    return []

                page.click(input_sel)
                page.wait_for_timeout(200)
                # Digita con delay 80ms per simulare digitazione umana
                page.type(input_sel, query, delay=80)

                # 4. Attendi comparsa risultati (senza premere invio)
                for res_sel in [
                    ".results .result",
                    "a.livesearch-item",
                    ".results",
                    ...
                    try:
                        page.wait_for_selector(res_sel, timeout=6000)
                        break
                    except Exception:
                        pass
                ...
                for item in raw:
                    ...
                    if not l.startswith("http"):
                        l = _BASE_AU + ("" if l.startswith("/") else "/") + l
                    results.append({
                        "title":     t,
                        "link":      l,
                        "raw_title": t,
