...
def _au_open_searchbar(page, ctx=None) -> bool:
    ...
    AnimeUnity ha due pulsanti identici: uno mobile (nascosto) e uno desktop.
    query_selector_all + is_visible() prende quello giusto.
    """
    for sel in BTN_SELS:
        try:
            els = page.query_selector_all(sel)   # ← TUTTI, non solo il primo
            for el in els:
                try:
                    if el.is_visible():           # ← solo quello visibile
                        return el, sel
                except Exception:
                    pass
        except Exception:
