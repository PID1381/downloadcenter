...
def search_animeunity(query: str, silent: bool = False) -> List[Dict]:
    ...
    AnimeUnity usa Vue.js con livesearch reattiva:
      - Digita nel campo <input class="search-bar"> 
      - I risultati appaiono in <div class="results"> senza submit
      - Struttura risultato: .livesearch-item > .livesearch-title + .livesearch-info
    ...
    Returns:
        Lista dict: {title, link, year, type, episodes, raw_title}
    ...
    # Tenta anche fast-path con requests (Inertia.js /archivio?title=)
    ...
    # Fallback Playwright: livesearch reattiva
    results: List[Dict] = []
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                if not search_input:
                    if not silent:
                        show_warning("Campo ricerca AnimeUnity non trovato.")
                    browser.close()
                    return []

                # Clicca e digita il titolo (simula digitazione umana)
                page.click(search_input)
                page.wait_for_timeout(300)
                page.type(search_input, query, delay=80)

                # Attendi che appaiano i risultati livesearch
                for res_sel in [".results .result", ".livesearch-item", ".results"]:
                    try:
                        page.wait_for_selector(res_sel, timeout=5000)
                        break
                    except Exception:
                        pass
