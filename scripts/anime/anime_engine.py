...
def search_animeunity(query: str, silent: bool = False,
    ...
    try:
        ...
        # ── JS per DOM scraping (zero backslash → zero SyntaxWarning) ────────
        _JS_DOM = (
            "() => {var AP = '/anime/';var RY = new RegExp('(19|20)[0-9]{2}');var RT = new RegExp('(TV|Movie|OVA|ONA|Special)', 'i');var RE = new RegExp('([0-9]+)[ ]*ep', 'i');var res = [];var seen = {};var links = Array.from(document.querySelectorAll('a'));links = links.filter(function(a) { return a.href && a.href.indexOf(AP) > -1; });links.forEach(function(a) {  var h = a.href || '';  if (!h || seen[h]) return;  seen[h] = true;  var c = a.parentElement || a;  for (var j = 0; j < 6; j++) {    var cn = c.className || '';    if (c.tagName === 'ARTICLE' || cn.indexOf('card') > -1 || cn.indexOf('item') > -1) break;    c = c.parentElement || c;  }  var te = c.querySelector('h3,h4,h5') || a;  var tt = (te.textContent || '').trim() || a.title || '';  var ct = (c.textContent || '').trim();  var ym = ct.match(RY);  var tm = ct.match(RT);  var em = ct.match(RE);  res.push({    title: tt, link: h,    year: ym ? ym[0] : '',    type: tm ? tm[0] : '',    episodes: em ? em[1] : ''  });});return res;}"
        )
        ...
        def _extract_items(obj, depth=0):
            ...
            if isinstance(obj, dict):
                for k in ("data", "records", "anime", "results", "items", "props"):
        ...
        def _on_resp(resp) -> None:
            try:
                ...
                if body and len(body) > 30:
                    _json_caps.append({"url": resp.url, "raw": body})
            except Exception: pass
        ...
        with sync_playwright() as pw:
            ...
            # Cattura JSON da tutte le schede (incluse quelle aperte dal sito)
            ctx.on("page", lambda p: p.on("response", _on_resp))
            page = ctx.new_page()
            ...
            try:
                ...
                # wait_until="commit" → non blocca se la pagina si chiude subito
                try:
                ...
                # Attendi che le API vengano chiamate (anche se la pagina si chiude)
                _safe_w(page, 5000)

                # Trova la pagina corretta tra tutte quelle vive del contesto
                ...
                if _AU_DEBUG and not silent:
                    ...
                    show_info(f"[AU_DEBUG] JSON intercettati: {len(_json_caps)}")
                    for _dbg in _json_caps:
                        print(f"[AU_DEBUG] JSON URL: {_dbg['url']}")
                        print(f"[AU_DEBUG] JSON raw (primi 500 char): {_dbg['raw'][:500]}")

                # ── Metodo 1: dati dall'API JSON intercettata ─────────────────
                for _jc in _json_caps:
                    try: _d = _json_fb.loads(_jc["raw"])
                    ...
                    for _a in _items:
                        ...
                        _lnk = (f"{_BASE_AU}/anime/{_id}-{_sl}" if _sl
                                else f"{_BASE_AU}/anime/{_id}")
                        results.append({
                            "title": _t, "link": _lnk, "raw_title": _t,
                    ...
                    if results:
                        if _AU_DEBUG and not silent:
                            show_info(f"[AU_DEBUG] risultati da JSON API: {len(results)}")
                        break

                # ── Metodo 2: DOM scraping ────────────────────────────────────
                if not results:
                    # Riapri pagina se chiusa
                    ...
                    if _pg_alive(page):
                        try:
                            ...
                            if _AU_DEBUG and not silent:
                                show_info(f"[AU_DEBUG] risultati DOM: {len(raw)}")
                            for item in raw:
                        ...
                        show_info("[AU_DEBUG] pagina chiusa, DOM skip")
