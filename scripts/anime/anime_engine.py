...
def _au_open_searchbar(page, ctx=None) -> bool:
    """
    Apre il campo di ricerca AnimeUnity.

    Comportamento reale del sito (Vue.js):
      - Click 1 NATIVO sulla lente -> apre scheda pub (YouTube/ad)
      - Chiudi SUBITO la scheda pub con expect_popup()
      - Click 2 NATIVO sulla lente -> appare input.search-bar

    NOTA: il click deve essere nativo (el.click()), NON via JS
    dispatchEvent, perche Vue.js ignora gli eventi sintetici.

    NOTA 2: ctx.on("page") globale NON deve essere attivo durante
    questa funzione, altrimenti chiude il popup prima che
    expect_popup() lo catturi.
    """
    INPUT_SELS = [
        "input.search-bar",
    ...
    def _find_lente():
        """Ritorna (elemento, selettore) del pulsante lente o (None, None)."""
        sels = [
        ...
        for sel in sels:
            try:
                ...
                pass
        # fallback: icona fa-search -> risali al button
        try:
            ico = page.query_selector("i.fa-search, .fas.fa-search")
            if ico:
    ...
    lente_el, lente_sel = _find_lente()
    if not lente_el:
        if _AU_DEBUG:
    ...
    if _AU_DEBUG:
        print(f"  [AU_DEBUG] pulsante lente trovato: {lente_sel}")

    # CLICK 1: click nativo, cattura popup con expect_popup
    if _AU_DEBUG:
        print("  [AU_DEBUG] click 1 nativo (atteso popup pub)...")
    try:
        with page.expect_popup(timeout=5000) as popup_info:
            lente_el.scroll_into_view_if_needed()
            lente_el.click()
        ...
        try:
            ...
            pass
        if _AU_DEBUG:
            print(f"  [AU_DEBUG] popup chiuso: {popup_url}")
        try:
            page.bring_to_front()
        except Exception:
            pass
        ...
        # Nessun popup -> il click potrebbe aver gia aperto il campo
        if _AU_DEBUG:
            print("  [AU_DEBUG] nessun popup al click 1 (continuo)")
        page.wait_for_timeout(300)
    ...
    # CLICK 2: deve aprire il campo di ricerca
    if _AU_DEBUG:
    ...
    if lente_el:
        try:
            ...
            lente_el.click()
            if _AU_DEBUG:
                print(f"  [AU_DEBUG] click 2 eseguito su: {lente_sel}")
        except Exception as _ex:
            if _AU_DEBUG:
                print(f"  [AU_DEBUG] errore click 2: {_ex}")
    else:
        if _AU_DEBUG:
            print("  [AU_DEBUG] pulsante lente non trovato per click 2")
...
def search_animeunity(query: str, silent: bool = False) -> List[Dict]:
    ...
    Flusso:
      1. Fast-path requests+BS4 (/archivio?title= Inertia.js JSON)
      2. Fallback Playwright:
         a. goto homepage + wait 2500ms Vue.js
         b. ctx.on("page") listener chiude popup automaticamente
         c. _au_open_searchbar(page, ctx) -> clicca lente + blocca ads
         d. _au_close_popups(ctx) + bring_to_front()
         e. type(query, delay=80ms) -> risultati live
         f. _au_livesearch_js() -> legge .results .result
    
    Debug: AU_DEBUG=1 -> headless=False, slow_mo=200ms, screenshot
    
    Returns: Lista dict {title, link, raw_title, year, type, episodes}
    """
    ...
    try:
        with sync_playwright() as pw:
            ...
            # Chiudi automaticamente ogni nuova scheda aperta dal sito
            # RIMOSSO: ctx.on('page') globale chiudeva il popup prima di expect_popup()
            ...
            try:
                ...
                # 2. Chiudi popup gia aperti al caricamento
                _au_close_popups(ctx)

                # 3. Clicca pulsante lente -> apre input.search-bar
                if _AU_DEBUG and not silent:
                    show_info("[AU_DEBUG] apertura campo ricerca...")
                bar_open = _au_open_searchbar(page, ctx)
                if not bar_open:
                    if not silent:
                        show_warning(
                ...
                # 4. Chiudi popup aperti dal click sulla lente
                _au_close_popups(ctx)
                try:
                    page.bring_to_front()
                except Exception:
                ...
                # 5. Trova il campo input (ora visibile)
                search_input = None
                for sel in [
                    "input.search-bar",
                    "input[placeholder*='Cerca']",
                ...
                # 6. Clicca e digita (80ms delay = digitazione umana)
                ...
                # 7. Attendi comparsa risultati (senza premere invio)
