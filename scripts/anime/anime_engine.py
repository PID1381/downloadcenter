...
def search_animeworld(query: str, silent: bool = False) -> List[Dict]:
    ...
    try:
        ...
        for item in film_list.find_all("div", class_="item"):
            ...
            if status_div:
                _badges = []
                for _d in status_div.find_all("div"):
                    for _c in _d.get("class", []):
                        if _c in ("dub", "movie", "ona", "ova", "special"):
                            _badges.append(_c.upper())
                if _badges:
                    if "DUB" in _badges:
                        category = "ITA"
                    elif "MOVIE" in _badges:
                        category = "MOVIE"
                    else:
                        category = _badges[0]
...
# ════════════════════════════════════════════════════════════════════════════
# §9  BROWSER STUBS — Playwright opzionale
# ════════════════════════════════════════════════════════════════════════════

class PageSession:
    """Wrapper Playwright opzionale. Ritorna vuoto se non installato."""
    def __init__(self):
        self._pw = None
        self._browser = None
    ...
    def __enter__(self):
        if HAS_PLAYWRIGHT:
            try:
                ...
                self._browser = self._pw.chromium.launch(headless=get_headless_mode())
            except Exception:
                pass
        return self
    ...
    def __exit__(self, *args):
        try:
            ...
            if self._pw:      self._pw.__exit__(*args)
        except Exception:
            pass

    def fetch_all_episodes(self, url: str):
        if not HAS_PLAYWRIGHT or not self._browser:
            return "", []
        try:
            ...
            return title, extract_episode_links(soup, url)
        except Exception:
            return "", []


def extract_video_links(page) -> List[str]:
    links: List[str] = []
    try:
        html = page.content()
        if not HAS_BS4 or BeautifulSoup is None:
...
def extract_from_js(html: str) -> List[str]:
    links: List[str] = []
    try:
        for m in re.finditer(
            r"""['"](https?://[^'"]+\.(?:mp4|mkv|m3u8|ts))['"]""",
            html, re.I
        ):
...
def extract_episode_links(soup, base_url: str = "") -> List[str]:
    ...
    try:
        ...
        if widget:
            server = (
                widget.find("div", class_="server active")
                or widget.find("div", class_="server")
            )
            if server:
                for li in server.find_all("li", class_="episode"):
                    a = li.find("a", href=True)
                    if a:
                        href = a["href"].strip()
                        if href:
                            links.append(normalize_url(href, base_url or BASE_URL))
        if not links:
            seen: Set[str] = set()
            for a in soup.find_all("a", href=re.compile(r"/ep/\d+", re.I)):
        ...
        pass
    return sort_links_numerically(links)
