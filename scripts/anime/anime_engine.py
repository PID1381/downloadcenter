    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                page.goto(_archivio_url, wait_until="domcontentloaded")
                page.wait_for_timeout(3000)

                # Chiudi popup/cookie banner se presenti
                ...
                # Attendi che Vue.js carichi almeno una card risultato
                _card_sels = [
                    "div.anime-card",
                    "article.anime-card",
                    "a.anime-card",
                ...
                if not _found_sel and _AU_DEBUG and not silent:
                    show_info("[AU_DEBUG] nessun selector card trovato, leggo DOM comunque")

                page.wait_for_timeout(800)

                # Leggi risultati via JS dal DOM Vue renderizzato
                raw = page.evaluate("""
                    () => {
                        const results = [];
                        const selectors = [
                        ...
                        for (const sel of selectors) {
                            ...
                            cards = cards.filter(a => /\/anime\/\d+/.test(a.href));
                        ...
                        for (const a of cards) {
                            ...
                            const titleEl = card.querySelector('h3,h4,h5,.title,.name,[class*="title"],[class*="name"]') || a;
                            const title = titleEl.textContent.trim() || a.title || a.textContent.trim();
                            const infoEl = card.querySelector('.info,.meta,.badge,[class*="info"],[class*="meta"]');
                ...
                if _AU_DEBUG and not silent:
                    show_info(f"[AU_DEBUG] risultati /archivio JS: {len(raw)}")
