...
def search_animeunity(query: str, silent: bool = False,
    ...
    """
    # ── Menu filtri interattivo (mostrato solo se nessun filtro passato) ──
    if not silent and genre is None and year is None and status is None \
        ...
        print("[Invio per cercare subito, oppure aggiungi filtri:]")
        _g  = input("  Genere    (lascia vuoto = tutti): ").strip()
        _a  = input("  Anno      (es. 2002, vuoto = tutti): ").strip()
        _s  = input("  Stato     (Terminato/In corso, vuoto = tutti): ").strip()
        _t  = input("  Tipo      (TV/Movie/OVA, vuoto = tutti): ").strip()
        _se = input("  Stagione  (Inverno/Primavera/Estate/Autunno, vuoto = tutti): ").strip()
        if _g:  genre      = _g
        if _a:  year       = _a
        if _s:  status     = _s
        if _t:  anime_type = _t
