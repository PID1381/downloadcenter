...
# ============================================================================
# AGGIORNAMENTO SILENZIOSO - per main_menu.py STEP 3
# ============================================================================

def check_aggiornamenti_silenzioso() -> list:
    """
    Scraping silenzioso: nessun output visivo.
    Ritorna lista di dict per gli anime con nuovi episodi:
      [{"titolo": "...", "nuovi": 2, "da": 10, "a": 12}, ...]
    Aggiorna anche il JSON watchlist_in_corso.json se trova nuovi ep.
    """
    try:
        ...
        for anime in watchlist_in_corso:
            try:
                result = check_nuovi_episodi_animeworld_silent(anime)
...
def check_nuovi_episodi_animeworld_silent(anime: dict) -> dict:
    """
    Versione silenziosa di check_nuovi_episodi_animeworld():
    nessun print, solo ritorna il risultato.
    """
    try:
        import requests
        from bs4 import BeautifulSoup
    except ImportError:
        return {"titolo": anime.get("titolo", ""), "episodi_nuovi": False, "nuovi_episodi": 0}
    ...
    try:
        ...
        ep_str        = anime.get("episodi", "[0/?]")
        ep_attuali    = extract_episode_number_from_string(ep_str)
        ep_totale_att = extract_total_episodes_from_format(ep_str)

        if not link:
            return {"titolo": titolo, "episodi_nuovi": False, "nuovi_episodi": 0,
                    "ep_attuali": ep_attuali, "ep_totale": ep_totale_att}
        ...
        ep_totali = len(server.find_all("li", class_="episode"))
        if ep_totali > ep_attuali:
            return {"titolo": titolo, "episodi_nuovi": True,
                    "nuovi_episodi": ep_totali - ep_attuali,
                    "ep_attuali": ep_attuali, "ep_totale": ep_totali}
        return {"titolo": titolo, "episodi_nuovi": False, "nuovi_episodi": 0,
                "ep_attuali": ep_attuali, "ep_totale": ep_totali}
