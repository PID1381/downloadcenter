...
# -*- coding: utf-8 -*-
"""
ricerca_scheda_anime.py v4.1
Download Center - scripts/anime/ricerca_scheda_anime.py

NOVITA v4.1 (rispetto a v4.0):
  [FIX]     Rimosso page.on("response") + page.type(delay=150ms)
            che causavano il problema "devo cliccare Aggiorna" per vedere
            i risultati. Il listener intercettava la POST AJAX vuota
            iniziale invece dei risultati reali.
  [MANTIENE] FIX 1  — URL dinamici via _get_base_url() / _get_search_url()
  [MANTIENE] FIX 2  — urlencode corretto con chiave decodificata
  [MANTIENE] FIX 3  — Parser HTML a 3 livelli (thumbnail > card > link-scan)
  [MANTIENE] FIX 4  — _unwrap_json(): estrae HTML da JSON wrapper AJAX
  [MANTIENE] FIX 5  — _http_search(): fallback HTTP puro con CookieJar
  [MANTIENE] FIX 6  — Titolo da data-content > h5 > img[alt] > caption
  [MANTIENE] FIX 7  — _filter_by_relevance(): filtra falsi positivi
  [MANTIENE] FIX 8  — Voto: regex solo decimali + validazione range 0<v<=10
  [VELOCITA] wait_until="domcontentloaded" ovunque (non networkidle)
  [VELOCITA] Timeout wait_for_timeout ridotti al minimo necessario
  [VELOCITA] GET diretto come strategia 1, form come fallback (come v3.5)
  [VELOCITA] wait_for_selector timeout ottimizzati per contesto
"""
...
class AnimeTracker:
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    direct_url = f"{search_url}?{params}"
                    if not silent:
                    ...
                    try:
                        ...
                        if not silent:
                            show_warning(f"Strategia GET fallita ({e1}); provo form...")
    ...
    def _parse_details(self, soup, url: str) -> dict:
        ...
        FIX v4.0: voto regex solo decimali + validazione range.
        FIX v4.0: URL copertina dinamico via _get_base_url().
        """
        base_url = _get_base_url()
        d = {
...
def handle_ricerca_scheda_anime(tracker=None):
    """Menu principale ricerca scheda anime v4.1"""
    if tracker is None:
        tracker = AnimeTracker()

    if not HAS_PLAYWRIGHT:
        clear_screen()
        show_header("RICERCA SCHEDA ANIME v4.1")
    ...
    while True:
        clear_screen()
        print("  " + "=" * _W)
        print("  RICERCA SCHEDA ANIME v4.1")
        print("  " + "=" * _W)
        print()
