...
class AnimeTracker:
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    # ── 5. Naviga direttamente all'URL GET di ricerca ─────────
                    # AnimeClick usa GET con search_manga[title]=<query>
                    # Il submit del form causa una pagina "Oops" → usiamo GET diretto
                    import urllib.parse as _urlparse
                    search_base = "https://www.animeclick.it/ricerca/anime"
                    search_params = _urlparse.urlencode({"search_manga[title]": query})
                    search_url = f"{search_base}?{search_params}"
                    if not silent:
                        print(f"  [->] Navigazione GET risultati: {search_url}")
                    page.goto(search_url, wait_until="networkidle", timeout=20_000)

                    # Aspetta il container risultati
                    try:
                        ...
                        if not silent:
                            print(f"  [ok] Container risultati trovato")
                    except Exception:
                        if not silent:
                            print(f"  [!] Container non trovato, leggo DOM comunque")
                        page.wait_for_timeout(2000)

                    # ── 6. Leggi DOM pagina risultati ────────────────────────
                    dom_html = page.content()
                    json_html = _unwrap_json(dom_html)
                    if not silent:
                        print(f"  [dbg] DOM risultati: {len(json_html)} chars")
    ...
    def _filter_by_relevance(self, results: list, query: str) -> list:
        ...
        def _score(title: str) -> int:
            t = title.lower()
            # Normalizza: rimuovi articoli e punteggiatura per match parziale
