        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    # ── 5. Click su 'Aggiorna' e attendi navigazione ──────────
                    ...
                    if not submit_clicked and not silent:
                        print("  [!] Nessun pulsante submit trovato")

                    # Attendi che la pagina carichi i risultati
                    try:
                    ...
                    if not silent:
                        print(f"  [dbg] POST AJAX intercettate: {len(received_responses)}")
                    ...
                    # ── 6. Scegli sorgente HTML ───────────────────────────────
                    json_html = ""

                    # Priorita 1: POST AJAX che contiene la query (se presente)
                    best = max(
                    ...
                    if best:
                        ...
                        if not silent:
                            print(f"  [ok] Risposta AJAX scelta: {len(json_html)} chars")
                    else:
                        # Priorita 2: leggi il DOM aggiornato dopo navigazione
                        if not silent:
                            print("  [dbg] Nessuna POST AJAX utile. Leggo DOM pagina...")
                        dom_html = page.content()
                        json_html = _unwrap_json(dom_html)
                        if not silent:
                            print(f"  [dbg] DOM: {len(json_html)} chars")
