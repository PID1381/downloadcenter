...
NOVITA v3.5 (allineamento engine v2.0):
  [FIX]      Anno hardcoded 1950-2025 -> range dinamico via datetime.date.today().year
  [REFACTOR] COOKIE_SEL / COOKIE_TEXTS locali rimossi -> importati da anime_engine
  [REFACTOR] _dismiss_cookies() usa COOKIE_SEL / COOKIE_TEXTS dell'engine
  [REFACTOR] textwrap importato in cima al file (non piu inline in export_scheda)
  [REFACTOR] get_headless_mode gia importato da engine (invariato)
  [MANTIENE] AnimeTracker completo: search_anime, get_anime_details, export_scheda
  [MANTIENE] handle_ricerca_scheda_anime: menu principale
"""
...
BASE_URL = get_url("anime", "animeclick")
SEARCH_PAGE_URL = BASE_URL + "/ricerca/anime"
SEARCH_PARAM    = "search_manga%5Btitle%5D"
SEARCH_INPUT_ID = "search_manga_title"
RESULTS_DIV_ID  = "row-elenco-opere"
_W              = 56
PAGE_TIMEOUT    = 20_000

# Anno corrente calcolato dinamicamente (fix: era hardcoded a 2025)
...
class AnimeTracker:
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    direct_url = f"{SEARCH_PAGE_URL}?{SEARCH_PARAM}={quote(query)}"
                    if not silent:
                    ...
                    if not results:
                        ...
                        try:
                            page.goto(SEARCH_PAGE_URL, wait_until="networkidle", timeout=PAGE_TIMEOUT)
                        except Exception:
                            page.goto(SEARCH_PAGE_URL, wait_until="domcontentloaded", timeout=PAGE_TIMEOUT)
                        page.wait_for_timeout(1_000)
                        self._dismiss_cookies(page)
    ...
    def _parse_search(self, soup) -> list:
        ...
        if not container:
            return []
        results = []
        for thumb in container.find_all("div", class_=re.compile(r"\bthumbnail\b")):
            caption = thumb.find("div", class_="caption")
            if not caption:
                continue
            a_el = caption.find("a", href=True)
            if not a_el:
                continue
    ...
    def _parse_details(self, soup, url: str) -> dict:
        ...
        Usa _YEAR_NOW = datetime.date.today().year come limite superiore.
        """
        d = {
        ...
        # FIX v3.5: range superiore calcolato con datetime.date.today().year
        # cosi non scarta anime dell'anno corrente (era hardcoded 2025)
        #
        # Strategia:
        #   1. Cerca etichetta esplicita "Anno" + anno nel testo
        #   2. Fallback: primo anno nel testo nel range valido
        #   Filtro range applicato programmaticamente (non nel regex)
        ...
        # Priorita 1: etichetta esplicita "Anno"
        ...
        if anno_label and _valid_year(anno_label.group(1)):
            ...
            # Priorita 2: primo anno nel testo nel range valido
    ...
    def export_scheda(self, details: dict, *, silent: bool = False) -> str:
        ...
        Nota: textwrap ora importato in cima al file (non piu inline).
        """
        ...
        try:
            ...
            # 2. Crea file .txt ───────────────────────────────────────────────
            # textwrap importato in cima al file (fix v3.5)
...
def handle_ricerca_scheda_anime(tracker=None):
    """Menu principale ricerca scheda anime v3.5"""
    if tracker is None:
        tracker = AnimeTracker()

    if not HAS_PLAYWRIGHT:
        clear_screen()
        show_header("RICERCA SCHEDA ANIME v3.5")
    ...
    while True:
        clear_screen()
        print("  " + "=" * _W)
        print("  RICERCA SCHEDA ANIME v3.5")
        print("  " + "=" * _W)
        print()
