...
# -*- coding: utf-8 -*-
"""
ricerca_scheda_anime.py v3.7
Download Center - scripts/anime/ricerca_scheda_anime.py

NOVITA v3.7 (fix ricerca POST + CSRF token):
  [FIX CRITICO] AnimeClick ignora completamente il parametro GET
                search_manga[title]=<query>: restituisce sempre i primi
                100 anime generici non filtrati.
                La ricerca reale funziona SOLO via:
                  1. Form POST con CSRF token (search_manga[_token])
                  2. Oppure chiamata fetch() JS con X-Requested-With: XMLHttpRequest
                Entrambi i metodi richiedono prima una GET per ottenere
                il token dalla pagina, poi una POST con token + cookie.
  [FIX]         search_anime() ora:
                  Step 1: GET /ricerca/anime → estrae CSRF token dal DOM
                  Step 2: compila il form e triggera submit jQuery
                          (attiva il listener ajaxFormRicerca)
                  Step 3: fallback fetch() JS POST diretto con token
                  Step 4: fallback Playwright form interattivo classico
  [MANTIENE]    Tutto il resto invariato rispetto a v3.6
                (URL dinamici, parser a 3 livelli, export, display)
"""
...
# ── Costanti non legate all'URL base ────────────────────────────────────────
...
def _get_search_url() -> str:
    """URL pagina ricerca anime (calcolato dinamicamente)."""
...
class AnimeTracker:
    ...
    def search_anime(self, query: str, *, silent: bool = False) -> list:
        ...
        FIX v3.7 — meccanica corretta:
        AnimeClick usa un form POST con CSRF token (search_manga[_token]).
        Il GET con ?search_manga[title]=<query> viene ignorato dal server:
        restituisce sempre i primi 100 anime generici non filtrati.

        Flusso:
          Step 1: GET /ricerca/anime → ottieni CSRF token + cookie sessione
          Step 2: trigger submit jQuery del form → ajaxFormRicerca carica risultati
          Step 3: fallback fetch() JS POST diretto con token e header AJAX
          Step 4: fallback form interattivo Playwright classico
        """
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    # ── Step 1: GET per ottenere CSRF token + cookie ──────────
                    if not silent:
                        print(f"  [->] GET {search_url} (recupero CSRF token)...")
                    try:
                    ...
                    # Verifica presenza del form e del token CSRF
                    csrf_token = ""
                    try:
                        csrf_token = page.eval_on_selector(
                            "input[name='search_manga[_token]']",
                            "el => el.value"
                        )
                        if not silent and csrf_token:
                            print(f"  [✓] CSRF token ottenuto: {csrf_token[:16]}...")
                    except Exception:
                        if not silent:
                            show_warning("CSRF token non trovato nel DOM.")

                    # ── Step 2: compila form + trigger submit jQuery ───────────
                    # AnimeClick usa ajaxFormRicerca() su #form-ricerca-opera.
                    # Il trigger jQuery 'submit' attiva il listener AJAX
                    # che carica i risultati in #elenco-opere senza reload.
                    if csrf_token:
                        if not silent:
                            print("  [->] Compilo form e triggero submit jQuery...")
                        try:
                            # Compila il campo titolo
                            page.fill("input[name='search_manga[title]']", query)
                            page.wait_for_timeout(300)

                            # Trigger submit via jQuery (attiva ajaxFormRicerca)
                            page.evaluate("""
                                (query) => {
                                    const form = document.getElementById('form-ricerca-opera');
                                    if (!form) return 'no_form';
                                    // Imposta il valore nel campo titolo
                                    const titleInput = form.querySelector(
                                        'input[name="search_manga[title]"]');
                                    if (titleInput) titleInput.value = query;
                                    // Trigger submit jQuery se disponibile
                                    if (typeof $ !== 'undefined') {
                                        $(form).trigger('submit');
                            ...
                            # Attendi caricamento risultati AJAX
                            try:
                            ...
                            if results and not silent:
                                print(f"  [✓] Step 2 OK: {len(results)} risultati")
                        except Exception as e2:
                            if not silent:
                                show_warning(f"Step 2 fallito ({e2})")

                    # ── Step 3: fetch() JS POST diretto con token ─────────────
                    # Usa le API fetch() del browser già autenticato (cookie
                    # di sessione presenti) per inviare la POST esattamente
                    # come farebbe il form, con header X-Requested-With: XMLHttpRequest.
                    if not results and csrf_token:
                        if not silent:
                            print("  [->] Fallback Step 3: fetch() JS POST diretto...")
                        try:
                            post_html = page.evaluate(
                                """
                                async ([q, token, searchUrl]) => {
                                    const params = new URLSearchParams();
                                    params.append('search_manga[title]',        q);
                                    params.append('search_manga[staff]',         '');
                                    params.append('search_manga[annoStagione]',  '');
                                    params.append('search_manga[tagsGenerali]',  '');
                                    params.append('search_manga[_token]',        token);
                                    try {
                                ...
                                """,
                                [query, csrf_token, search_url],
                            )
                            if post_html and len(post_html) > 100:
                                results = self._parse_search(
                                    BeautifulSoup(post_html, "html.parser")
                                )
                            if results and not silent:
                                print(f"  [✓] Step 3 OK: {len(results)} risultati")
                        except Exception as e3:
                            if not silent:
                                show_warning(f"Step 3 fallito ({e3})")

                    # ── Step 4: form interattivo Playwright classico ───────────
                    # Ultimo fallback: naviga, trova input, digita, premi Enter.
                    if not results:
                        if not silent:
                            print("  [->] Fallback Step 4: form interattivo Playwright...")
                        try:
                            page.goto(search_url, wait_until="domcontentloaded", timeout=PAGE_TIMEOUT)
                            page.wait_for_timeout(800)
                            self._dismiss_cookies(page)
                            ...
                            if filled:
                                ...
                                if results and not silent:
                                    print(f"  [✓] Step 4 OK: {len(results)} risultati")
                            elif not silent:
                                show_warning("Step 4: campo di ricerca non trovato.")
                        except Exception as e4:
                            if not silent:
                                show_error(f"Step 4 fallito ({e4})")
    ...
    def _parse_search(self, soup) -> list:
        ...
        Parser a tre livelli (v3.6+):
          Livello 1: .thumbnail dentro #row-elenco-opere (struttura classica)
          Livello 2: .card / .item / .opera (variazioni HTML)
...
def _select_result(results: list):
    """Menu selezione da lista risultati. Ritorna il dict scelto o None."""
    if not results:
...
def _display_details(d: dict) -> None:
    """Stampa a video i dati scheda anime con box-drawing."""
    rows = [
...
def handle_ricerca_scheda_anime(tracker=None):
    """Menu principale ricerca scheda anime v3.7"""
    if tracker is None:
        tracker = AnimeTracker()

    if not HAS_PLAYWRIGHT:
        clear_screen()
        show_header("RICERCA SCHEDA ANIME v3.7")
    ...
    while True:
        clear_screen()
        print("  " + "=" * _W)
        print("  RICERCA SCHEDA ANIME v3.7")
        print("  " + "=" * _W)
        print()
