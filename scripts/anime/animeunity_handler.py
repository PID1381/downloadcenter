...
# ── helper: rimuove caratteri illegali nei nomi file (Windows/Linux) ──────────
def _sanitize_fname(name: str) -> str:
    import re as _re
    cleaned = _re.sub(r'[\\/:*?"<>|]', '', name)
    cleaned = cleaned.strip(". ")
    return cleaned[:180] or "episodio"
...
def _offri_download(hls_url: str, titolo: str = "") -> None:
    ...
    if ask_yes_no("Avviare download diretto?"):
        try:
            ...
            fname = _sanitize_fname(titolo or "episodio") + ".ts"
...
if __name__ == "__main__":
    ...
    # ── opzione yt-dlp (PATCH v1.1) ──────────────────────────────────────────
    def _offri_ytdlp(self, url: str, titolo: str = "") -> None:
        """Download diretto con yt-dlp (best MP4, stessa cartella)."""
        import sys
        from pathlib import Path as _Path
        _dl_dir = _Path(__file__).parent.parent / "download"
        ...
        if not _w.check_installed():
            ...
            input("  Premi Invio...")
            return
        fname = _sanitize_fname(titolo) if titolo else None
        print(f"\n  ⬇  yt-dlp download…  →  {_w._get_download_dir()}")
        ok = _w.download_url(url, filename=fname)
        print()
        if ok:
            print("  ✅ Download completato (MP4).")
        else:
